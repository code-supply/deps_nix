defmodule DepsNixTest do
  ...
  property "creates fetch-only derivations for git deps marked app: false and compile: false" do
    check all dep <-
                dep(
                  name: :an_asset_dep,
                  scm: Mix.SCM.Git,
                  dep_opts: [app: false, compile: false]
                ) do
      converger = fn [env: :prod] -> [dep] end
      nix = output(%DepsNix.Options{envs: %{"prod" => :all}}, converger)

      assert nix =~ "an_asset_dep = stdenv.mkDerivation",
             "Couldn't find fetch-only fetcher for #{dep.app} in #{nix}"
    end
  end

  property "doesn't create derivations for non-git deps marked both app: false and compile: false" do
    check all dep <-
                dep(
                  name: :not_a_mix_dep,
                  scm: Mix.SCM.Hex,
